...
def get_engine() -> Engine:
    return create_engine(DB_URL + "?options=-csearch_path=dw", future=True)
...
def upsert_frame(df: pd.DataFrame, table: str, key_cols: list[str]):
    ...
    with engine.begin() as conn:
        ...
        df.head(0).to_sql(
            tmp, conn, schema="dw", if_exists="replace", index=False
        )
        ...
        merge_sql = (
            "INSERT INTO fact_reviews (" + ", ".join(all_cols) + ")\n"
            "SELECT " + ", ".join(all_cols) + f" FROM {tmp}\n"
